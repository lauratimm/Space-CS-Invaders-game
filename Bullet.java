...
public class Bullet extends Rectangle{
	...
	public boolean shooter (Avatar avatar, ArrayList<Enemy> enemyList, Pane layout, Heart heart,
			ArrayList<Character>heartList, Image heartImage) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (this.getType().equals("avatarBullet")){
				...
				// from the Pane, delete enemy from the list, and add one to number of enemies avatar has killed
					...
					avatar.setEnemiesKilled(avatar.getEnemiesKilled() + 1);
					}
				}
		...
		if (this.getBoundsInParent().intersects(avatar.getBoundsInParent())){
			if (avatar.getLife() > 0){
				avatar.loseLife();
				layout.getChildren().removeAll(heartList);
				heart.removeHeart(heartList, avatar, heartImage);
	...
	public boolean bossShooter (Avatar avatar, Boss boss, Pane layout, Heart heart, ArrayList<Character>heartList, Image heartImage) {
